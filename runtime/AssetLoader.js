...
export const Assets = {
  ...
  async loadAll() {
    ...
    if (failed > 0) {
      ...
      console.log(`[ASSETS] ✓ All ${loaded} sprites loaded successfully`);
    }
   ...
   */
  async loadItemIcon(iconPath) {
    if (!iconPath) return null;
    
    // Check if already loaded
    if (this.images[iconPath]) {
      return this.images[iconPath];
    }
    
    // Load the icon
    return new Promise((resolve) => {
      this.loadImage(iconPath, iconPath).then(() => {
        resolve(this.images[iconPath] || null);
      });
    });
  },
  
  /**
   * Get item icon (prefer PNG if available, fallback to emoji)
   */
  getItemIcon(item) {
    if (item.iconPath) {
      return this.images[item.iconPath] || null;
    }
    return null;
  }
};
